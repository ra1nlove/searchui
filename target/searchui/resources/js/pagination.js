jQuery.fn.pagination = function(maxentries, opts) {
	...
	return this.each(function() {
		...
		function pageSelected(page_id, evt) {
			...
			current_page = page_id;   //当前页码
			drawLinks();   //重绘页码
			var continuePropagation = opts.callback(page_id, panel);   //绑定到每个页码上的回调函数
		...
		function drawLinks() {
			panel.empty();    //清空
			var interval = getInterval();   //页码的起始
			var np = numPages();  //总页数
			...
			// 上一页    页码结构：上一页 1 2 ...interval[0]-intervaal[1] ... 99 100
		...
		$(this).on("click",".goto button",function(evt){
